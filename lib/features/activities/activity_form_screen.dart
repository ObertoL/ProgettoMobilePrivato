...
class _ActivityFormScreenState extends State<ActivityFormScreen> {
  ...
  void initState() {
    ...
    _costCtrl = TextEditingController(text: a?.estimatedCost?.toString() ?? '0.00');
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_isEditing) {
        ...
        final relatedExpenses = allExpenses.where((e) => e.activityId == updated.id).toList();

        for (final e in relatedExpenses){
          await expenseProvider.updateExpense(e.copyWith(title: updated.title, stageId: updated.stageId));
        }
        ...
        if (cost != null) {
          final activities = activityProvider.getByTrip(widget.tripId);
          final newActivity = activities.isNotEmpty ? activities.last : null;
          ...
          if (newActivity != null) {
            ...
            await expenseProvider.addExpense(
              ...
              activityId: newActivity.id, // COLLEGAMENTO CHIAVE TRA L'ATTIVITÀ E LA SPESA
